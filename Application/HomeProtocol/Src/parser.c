...
void HomeProtocol_ParserSetCallback(ParserContext_t *ctx,
    ParserCallback_t callback)
{
  ctx->callback = callback;
}

void HomeProtocol_ParserProcessBytes(ParserContext_t *ctx, unsigned char *data,
    size_t len)
{
  uint16_t i;
    ...
    if (ctx->in_body == 1) {
      *((unsigned char *) ctx->buffer + (ctx->byte_count++)) = data[i];
      ...
      if (ctx->byte_count == ctx->bytes_expected) {
        struct message *msg = (struct message *) pvPortMalloc(
            ctx->buffer->size);
        if (msg == NULL) {
          Error_Handler();
        }
