...
void ClientThread_SendMessage(message_t *message)
{
	if (xQueueOutMessages != NULL) {
		if (pdFALSE == xQueueSendToBack(xQueueOutMessages, (void *)&message, portMAX_DELAY)) {
			vPortFree(message);
		}
	}
}
...
int netconn_tls_send(void *ctx, const unsigned char *buf, size_t len)
{
	struct netconn *conn = (struct netconn *)ctx;
	size_t written;

	err_t ret = netconn_write_partly(conn, (void *)buf, len, NETCONN_COPY, &written);
...
void ClientThread_Main(const void * argument)
{
	...
	/* Load CA Certificate */
	ret = mbedtls_x509_crt_parse(&cacert, (const unsigned char *)CACERT, sizeof(CACERT) );
	...
	while (1) {
		ret = mbedtls_net_connect(&ssl_fd, HOMESERVER_HOST, HOMESERVER_PORT, MBEDTLS_NET_PROTO_TCP);
		...
		if (ret == 0) {
			...
			mbedtls_ssl_set_bio(&ssl, &ssl_fd, mbedtls_net_send, NULL, mbedtls_net_recv_timeout);
			...
			while (1) {
				if (bytes_remaining == 0) {
					if (pdTRUE == xQueueReceive(xQueueOutMessages, (void *)&in, 0)) {
						...
						memcpy((void *)(buffer_out + 2), (void *)in, MESSAGE_HEADER_SIZE + in->size);
